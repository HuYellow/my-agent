...
export type ItemKind = "userMessage" | "agentMessage" | "reasoning" | "toolCall" | "toolResult" | "commandExecution" | "fileChange" | "approvalRequest" | "approvalResult" | "error";
...
export type HarnessEvent = EventEnvelope<"thread/started", {
    thread: ThreadRecord;
}> | EventEnvelope<"turn/started", {
    turn: TurnRecord;
}> | EventEnvelope<"item/started", {
    item: ItemRecord;
}> | EventEnvelope<"item/delta", {
    itemId: string;
    delta: string;
}> | EventEnvelope<"item/completed", {
    item: ItemRecord;
}> | EventEnvelope<"approval/requested", {
    approval: PendingApproval;
    item: ItemRecord;
}> | EventEnvelope<"serverRequest/resolved", {
    approvalId: string;
    decision: "approve" | "reject";
}> | EventEnvelope<"turn/completed", {
    turn: TurnRecord;
}> | EventEnvelope<"turn/cancelled", {
    turn: TurnRecord;
    message: string;
}> | EventEnvelope<"turn/failed", {
    turn: TurnRecord;
    message: string;
}> | EventEnvelope<"skills/changed", {
    skills: SkillDescriptor[];
}>;
